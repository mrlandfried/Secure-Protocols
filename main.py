import random
""" main.py defines 13 functions for understanding RSA encryption.
...
version 1/29/19 - updated to work with Python3
"""
...
def make_keys_from_primes(p, q):
    ...
    while d*e == 1:
        ...
        if len(factors)>1: # if there is a pair of factors
            ...
            # Get the other key
            e = product/d
        d = int(d) #typecast d as int - required for Python3 due to natural division instead of integer division
        e = int(e) #typecast e as int - required for Python3 due to natural division instead of integer division
...
def crypt_number(nde, number_message):
    ...
    n, d_or_e = nde # unpack tuple into components - Required for Python3
...
def use_key(nde, number_message, chunk_size=4):
    ...
    for number in numbers:
        crypted = crypt_number(nde, int(number))
        crypted = str(crypted)
